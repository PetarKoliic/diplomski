...
export class AdminForumComponent implements OnInit {
  ...
  arts: TopicPagination = new TopicPagination(
    'art',
    0,
    this.page_size_options[0],
    this.page_size_options[0]
  );
  fairs: TopicPagination = new TopicPagination(
    'fair',
    0,
    this.page_size_options[0],
    this.page_size_options[0]
  );
  socials: TopicPagination = new TopicPagination(
    'social',
    0,
    this.page_size_options[0],
    this.page_size_options[0]
  ...
  search() {
    ...
    this.service.get_all_topics().subscribe((topics: Topic[]) => {
      ...
      this.arts.topics = this.arts.topics.filter((topic) =>
        topic.title.includes(this.search_topic)
      );
      this.socials.topics = this.socials.topics.filter((topic) =>
        topic.title.includes(this.search_topic)
      );
      this.fairs.topics = this.fairs.topics.filter((topic) =>
        topic.title.includes(this.search_topic)
      );
  ...
  pageEvent(event: PageEvent, topic_pagination: TopicPagination) {
    ...
    topic_pagination.refresh_show_topics(
      this.startIndex,
      this.endIndex,
      event.pageSize
    );
  ...
  load_topics() {
    ...
    this.service.get_all_topics().subscribe((topics: Topic[]) => {
      ...
      TopicPagination.refresh_all_topics(
        this.startIndex,
        this.endIndex,
        this.page_size_options[0]
      );
  ...
  delete(topic: Topic) {
    ...
    console.log('date_added : ' + topic.date_added);

    this.service
      .delete_topic(topic.title, topic.date_added)
      .subscribe((res: any) => {
